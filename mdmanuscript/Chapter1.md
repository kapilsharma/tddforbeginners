...
In this workshop, we are going to make a simple command line calculator. Our calculator will have just `add` and `product` functionality. Requirements are pretty simple but the main focus is on initial steps of `Test Driven Development`.
...
However, as in real projects, we will make it in parts. As in real projects, we have some initial set of requirements, we achieve them and make project live. Once the project is live, we get next set of requirements and start development for next version of the application. Here, we will divide our application is tasks. Please assume every task is an application, supposed to go live after development. This means every task must be complete in itself.

Once we complete a task, we need to start development of next task, which represents a new feature of our application. This means, doing new task should not break the previous task and this is where `test driven development` comes into the picture. We will write test of previous tasks, say task 1 and 2, and while working on task 3, PHP Unit will ensure Task 1 and 2 are still working.
...
> Please do not consider the scenarios not mentioned in the task statements. Do not complicate the situations. We will just do what task ask us to do.

## Task 1 solution

Let's code to achieve task 1 requirements. Before we jump to the solution, we have the following prerequisite for this task.
 ...
 - `Git and GitHub account`: We need Git installed locally and you must have HitHub account (free). If you are not aware of git and GitHub, `Appendix A: Git and Github basics` will help you.
 - `Composer`: We will also need the composer. If you are not aware of the composer, `Appendix B: Composer` will help you.

### Think well

I remember a story from an old Hindi movie (forgot movie name). There was a factory, where one machine was not working. Factory owner was tensed as if machine do not work for a single day, he will have loss of several thousand Rupees. Fortunately, an Engineer was visiting the factory and was asked to check if he can fix the machine. He asked a hammer and hit machine on one point, and the machine started. The owner was happy and asked him service charges, engineer asked Rs. 1000, which was pretty high at that time. The owner was shocked, Rs. 1000 just for one hammers hit? Engineer replied, "No, just Rs. 1 for hammer hit; Rs. 999 for where should I hit."

Similarly, if a developer works 8 hours a day, it generally means he think 6-7 hours and types 1-2 hours. This is obvious, we are developers, not the typist. We do not get payment for typing, but what should we type and this needs time to think.

Before we start writing first line of the code, we must think what we need to do? Do we know that? Well yes, task 1 requirements are pretty clear but wait, maybe not, we do not know how will we achieve that. So let's list down few requirements.
 ...
 - Our `Calculator` class will contain main business logic.
   - We write PHP Unit tests for business logic, means Calculator class must be tested.
 ...
 - Our `add` method will have business logic to add numbers so it must be tested. Which tests?
   ...
   - Add with single parameter should return the same number.
   ...
   - There are few failure conditions as well
     ...
     - Add with non-number within parameter should fail (throw an exception).

With above few bullet points, we have a rough road-map of we need to do. Remaining chapter is all about how we do it.

### Development folder

Although not part of the task, I want to recommend how to organize your projects. There must be a single folder say `dev` which must contains all of your projects. Within `dev` folder, you may keep all of your projects. If you work on too many projects, may create further sub-folder to arrange different type of projects.

`dev` folder may be kept on location like `/home/<yourname>/dev` on Linux/Mac or `D:\dev` on windows.

> This also means you should never keep your projects in `/var/www` folder. That is a bad practice. Create a project in dev folder and use virtual host if needed. I specially mentioned this as I saw many developers putting all of their projects in `/var/www` folder.
